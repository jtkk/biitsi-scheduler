...
I'm part of a group of friends who like to get together to play beach volleyball. Each week, a the group organizer looks at the available courts on the facility's website, inputs them into the Google Sheet, and then asks all the members to mark their availability. Then, if at least 4 members of the group are available on a given day, the organizer reserves the court on the facility's website.

My goal is to automate as much of this process as possible, so the person doing the scheduling doesn't have to do as much maintenance on the Google Sheet, since courts become available/unavailable constantly throughout the week.

To do this, I wanted to create something that would scrape the available court data from the Biitsi.fi website, then format it and insert sheets and rows into a Google Sheet.
